          ...
          <form className="mt-12 py-12" onSubmit={handleSubmit(processForm)}>
            {currentStep === 0 && (
              ...
              <div className="space-y-1">
                ...
                {/* numeric input bound to RHF */}
                <FormField
                                  label="Level"
                                  stepsReference="academicSchema.level"
                                  type="number"
                                />
                {errors.academicSchema?.level && (
                  <p className="text-red-600 text-sm">{errors.academicSchema.level.message}</p>
                )}
            ...
            {currentStep === 0 && (
              ...
              >
                ...
                {responsibilities.map((field, index) => (
                 // inside your {responsibilities.map((field, index) => ( … ))}
<div
  key={field.id}
  className="grid grid-cols-1 gap-6 sm:grid-cols-2"
>
  ...
  <div className="space-y-1">
    ...
    <select
      {...register(
        `responsibilitiesSchema.${index}.level` as const,
    ...
    {errors.responsibilitiesSchema?.[index]?.level && (
      <p className="text-red-600 text-sm">
        {errors.responsibilitiesSchema[index]!.level!.message}
      </p>
    )}
  </div>
  ...
  <div className="flex flex-col gap-2">
    ...
    <div className="flex items-center gap-2">
      <input
        ...
        {...register(`responsibilitiesSchema.${index}.toDate` as const)}
        className="block w-full p-2 border rounded-md"
      />
      <button
        type="button"
        onClick={() =>
          setValue(
            `responsibilitiesSchema.${index}.toDate` as const,
            "9999-12-12"
          )
        }
        className="text-blue-500 underline text-sm"
    ...
    >
      Remove
                ...
                {extracurriculars.map((field, index) => (
                  // inside your {extracurriculars.map((field, index) => ( … ))}
<div
  key={field.id}
  className="grid grid-cols-1 gap-6 sm:grid-cols-2"
>
  ...
  {/* ↓ LEVEL DROPDOWN + “Other” TEXT INPUT ↓ */}
  {(() => {
    // get the current value
    const lvl = watch(
      `extracurricularsSchema.${index}.level`
    ) as string;
    return (
      <div className="space-y-1">
        ...
        <select
          {...register(
            `extracurricularsSchema.${index}.level` as const,
            { required: "Please select a level" }
          )}
          className="w-full p-2 border rounded-md focus:ring-blue-500"
        >
        ...
        {errors.extracurricularsSchema?.[index]?.level && (
          <p className="text-red-600 text-sm">
            {errors.extracurricularsSchema[index]!.level!.message}
          </p>
        )}
        {lvl === "Other" && (
          <div>
            ...
            <input
              type="text"
              {...register(
                `extracurricularsSchema.${index}.otherLevel` as const,
                { required: "Please specify level" }
              )}
              className="w-full p-2 border rounded-md focus:ring-blue-500 mt-1"
            />
            {errors.extracurricularsSchema?.[index]?.otherLevel && (
              <p className="text-red-600 text-sm">
                {errors.extracurricularsSchema[index]!.otherLevel!
                  .message}
              </p>
