      ...
      const fetchData = async () => {
        try {
          ...
          if (!role || role !== "principal") {
            throw new Error("Unauthorized access. Only Principal can view this page.");
          }
  ...
  if (loading) {
    return (
      <Layout moduleType="principal">
        <div className="container mx-auto p-4">Loading principal dashboard...</div>
      </Layout>
    );
  }
      ...
      <div className="container mx-auto p-4">
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="bg-blue-500 text-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            <h3 className="text-xl font-bold mb-1">{principalData?.personalDetails?.principal_name || 'Principal'}</h3>
            <p className="text-base">{principalData?.personalDetails?.designation || 'Institution Head'}</p>
          ...
          {/* Awards Card */}
          <div className="bg-blue-500 text-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            <h3 className="text-3xl font-bold mb-1">{principalData?.awards?.length || 0}</h3>
              ...
              {principalData?.awards && principalData.awards.length > 0 && (
                <>
                  ...
                  <span className="block text-base text-blue-200">
                    This Year: {getThisYearCount(principalData.awards, "awardDate")}
                  </span>
                </>
              )}
            </p>
          ...
          <div className="bg-blue-500 text-white p-6 rounded-xl shadow-md hover:shadow-lg transition-shadow">
            <h3 className="text-3xl font-bold mb-1">{principalData?.eventOrganized?.length || 0}</h3>
            <p className="text-base">
              Events Organized
              {principalData?.eventOrganized && principalData.eventOrganized.length > 0 && (
                <>
                  <span className="block text-base mt-1 text-blue-200">
                    Latest: {getLatestYear(principalData.eventOrganized, "toDate")}
                  </span>
                  <span className="block text-base text-blue-200">
                    This Year: {getThisYearCount(principalData.eventOrganized, "toDate")}
                  </span>
                </>
              )}
            </p>
          ...
          {principalData?.departmentStats && principalData.departmentStats.length > 0 && (
              ...
              <div className="grid grid-cols-4 gap-4">
                {principalData.departmentStats.map((dept, index) => (
                  <div key={index} className="bg-blue-600 p-4 rounded-lg">
                    <h4 className="text-lg font-semibold">{dept.departmentName}</h4>
