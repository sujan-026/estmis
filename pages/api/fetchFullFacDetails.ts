...
export default async function handler(req, res) {
  ...
  try {
    ...
    const personalDetailsQuery = `
      SELECT 
        ...
        [bankName], [accountNo], [accountName], [accountType], [branch], [ifsc], [pfNumber],
        [uanNumber], [pensionNumber], [motherName], [fatherName], [spouseName], [children],
        [dateOfJoiningDrait], [designation], [aided], [maritalstatus]
      FROM [aittest].[dbo].[facultyPersonalDetails]
      WHERE [employee_id] = @employee_id
    `;
 ...
 const employeeMoocQuery = `
    ...
    // …later, after professionalMembersResult …
    const employeeMoocResult = await pool
      .request()
      .input("employee_id", sql.NVarChar, employee_id)
      .query(employeeMoocQuery);
    ...
    const response = {
      ...
      employeeMoocs:           employeeMoocResult.recordset || [],
    };
    return res.status(200).json(response);
  } catch (error) {
    console.error("Error fetching faculty details:", error);
