...
// API handler for faculty academic details
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      console.log("POST/PUT payload:", req.body);

      const operations = [
        {
          schema: academicSchema ? [academicSchema].filter(Boolean) : [],
          query: `
          ...
          params: (item) => [
            { name: "employee_id", type: sql.NVarChar, value: facultyId },
            {
              name: "level",
              type: sql.Int,
              value: item.level && !isNaN(parseInt(item.level)) ? parseInt(item.level) : 0,
            },
          ],
        },
        {
        ...
        },
        {
          ...
          params: (item) => [
            ...
            { name: "awardDate", type: sql.Date, value: item.awardDate ? new Date(item.awardDate) : null },
            { name: "awardFrom", type: sql.NVarChar, value: item.awardFrom || "Unknown" },
            { name: "recognitionorawardDate", type: sql.Date, value: item.recognitionorawardDate ? new Date(item.recognitionorawardDate) : null },
          ],
        },
        {
          ...
          params: (item) => [
            ...
            { name: "recognitionorawardReceived", type: sql.NVarChar, value: item.recognitionorawardReceived || "Unknown" },
            { name: "recognitionorawardFrom", type: sql.NVarChar, value: item.recognitionorawardFrom || "Unknown" },
            { name: "awardReceived", type: sql.NVarChar, value: "Unknown" },
            { name: "awardDate", type: sql.Date, value: null },
            { name: "awardFrom", type: sql.NVarChar, value:"Unknown" },
            { name: "recognitionorawardDate", type: sql.Date, value: item.recognitionorawardDate ? new Date(item.recognitionorawardDate) : null },
          ],
        },
        {
...

{
  ...
  query: `
    ...
    WHEN MATCHED THEN
      UPDATE
        SET eventType   = source.eventType,
    ...
    WHEN NOT MATCHED THEN
      INSERT (
        employee_id, eventType, eventTitle,
        fromDate, toDate, organizer, level, achievement
      )
      VALUES (
        source.employee_id, source.eventType, source.eventTitle,
        source.fromDate, source.toDate, source.organizer, source.level, source.achievement
      );
  `,
  params: (item) => {
    // if user chose “Other”, use their custom text
    const actualLevel =
      item.level === "Other"
        ? (item.otherLevel?.trim() || "Other")
        : item.level || "Unknown";
    ...
    return [
      ...
      { name: "level",        type: sql.NVarChar, value: actualLevel },
      { name: "achievement",  type: sql.NVarChar, value: item.achievement  || "Unknown" },
    ];
  }
},

// … next (OutreachActivity) operation …
      ...
      for (const operation of operations) {
        for (const item of operation.schema) {
          try {
            ...
            console.error(`Error in ${operation.query}:`, err);
            throw new Error(`Failed to process operation: ${err.message}`);
          }
        }
      }
      ...
      // Ensure correct table name for addtionalResponsibility
      const correctedTable = table === "AdditionalResponsibility" ? "addtionalResponsibility" : table;
      const deleteQuery = `
        DELETE FROM [aittest].[dbo].[${correctedTable}]
        WHERE id = @id;
      `;
