...
for (const extracurricular of extracurricularsSchema) {
  // pick the “real” level value:
  const actualLevel =
    extracurricular.level === "Other"
      ? extracurricular.otherLevel?.trim() || "Unknown"
      : extracurricular.level || "Unknown";
  ...
  await pool
    ...
    .input(
      "eventType",
      sql.NVarChar,
      extracurricular.typeOfEvent || "Unknown"
    )
    .input(
      "eventTitle",
      sql.NVarChar,
      extracurricular.titleOfEvent || "Unknown"
    )
    .input(
      "fromDate",
      sql.Date,
      extracurricular.fromDate
        ? new Date(extracurricular.fromDate)
        : new Date()
    )
    .input(
      "toDate",
      sql.Date,
      extracurricular.toDate
        ? new Date(extracurricular.toDate)
        : new Date()
    )
    .input(
      "organizer",
      sql.NVarChar,
      extracurricular.organizer || "Unknown"
    )
    // use actualLevel instead of raw dropdown value
    .input("level", sql.NVarChar, actualLevel)
    .input(
      "achievement",
      sql.NVarChar,
      extracurricular.achievement || "Unknown"
    )
