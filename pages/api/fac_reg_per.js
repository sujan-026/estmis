...
const actualReligion =
      personalSchema.religion === "Other"
        ? personalSchema.Religion
        : personalSchema.religion;
    const actualCaste =
      personalSchema.caste === "Others"
        ? personalSchema.Caste
        : personalSchema.caste;
    const actualCategory =
      personalSchema.category === "Others"
    ...
    const insertPersonalDetailsQuery = `
      INSERT INTO [aittest].[dbo].[facultyPersonalDetails] (
        ...
        accountType, branch, ifsc, pfNumber, uanNumber, pensionNumber, motherName, fatherName,maritalstatus, 
    ...
    await pool
     ...
     .input("religion", sql.NVarChar, actualReligion ?? "Hindu")
      .input("caste", sql.NVarChar, actualCaste ?? "General")
      .input("category", sql.NVarChar, actualCategory ?? "General") 
      .input(
        "motherTongue",
        sql.NVarChar,
      ...
      .input("maritalstatus", sql.NVarChar, dependentsSchema?.maritalstatus ?? "Unknown")
      .input(
        "spouseName",
        sql.NVarChar,
        dependentsSchema?.spouseName ?? "Not Applicable"
      )
      .input("children", sql.NVarChar, dependentsSchema?.children ?? "0")
      .input(
        "dateOfJoiningDrait",
        sql.Date,
    ...
    const insertFacultyEducationQuery = `
      INSERT INTO [aittest].[dbo].[facultyEducation] (
        employee_id, Program, regNo,university, schoolCollege, specialization, 
    ...
    for (const education of facultyEducationData) {
      await pool
        ...
        .input("university", sql.NVarChar, education.university || "Unknown")
        .input(
          "schoolCollege",
          sql.NVarChar,
