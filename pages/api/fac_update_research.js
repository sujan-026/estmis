...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === "GET") {
      ...
      const queries = {
        ...
        `,
        EmployeeMooc: `
          SELECT [id] AS id, [employee_id], [title], [typeofMooc], [duration], 
                 [FDP], [result], [grade]
          FROM [aittest].[dbo].[EmployeeMooc]
          WHERE [employee_id] = @employee_id;
        `,
      };
      ...
      const operations = [
        ...
        },
        {
          ...
          params: (item) => [
            { name: "id", type: sql.Int, value: item.id ? Number(item.id) || null : null },
            { name: "employee_id", type: sql.NVarChar, value: facultyId },
            { name: "Research_Supervisor", type: sql.NVarChar, value: item.research_supervisors || null },
        ...
        },
        {
          ...
          query: `
            ...
            USING (SELECT @id AS id, @employee_id AS employee_id, @sanctionedDate AS sanctionedDate,
                          ...
                          @coPrincipalInvestigator AS coPrincipalInvestigator, @status AS status ,@title AS title) AS source
            ON target.id = source.id
            WHEN MATCHED THEN
              UPDATE SET 
            ...
            WHEN NOT MATCHED THEN
              INSERT (employee_id, sanctionedDate, projectPeriod, amount, principalInvestigator,
                     coPrincipalInvestigator, status , title)
          ...
          query: `
            ...
            WHEN MATCHED THEN
              UPDATE SET 
                ...
                authors = source.authors
            WHEN NOT MATCHED THEN
              INSERT (employee_id, areaOfResearch, grantedYear, patentNo, patentStatus, authors)
              VALUES (source.employee_id, source.areaOfResearch, source.grantedYear,
                     source.patentNo, source.patentStatus, source.authors);
          `,
          params: (item) => [
            { name: "id", type: sql.Int, value: item.id ? Number(item.id) || null : null },
            { name: "employee_id", type: sql.NVarChar, value: facultyId },
  ...
  query: `
    ...
    WHEN NOT MATCHED THEN
      ...
      VALUES (
        source.fromDate, source.toDate, source.organizer, source.venue, source.sponsor,
        source.targetAudience, source.employee_id, source.nameofevent, source.typeofevent
      );
  `,
  params: item => {
    const finalType = item.typeofevent === "Other"
      ? (item.otherType?.trim() || null)
      : (item.typeofevent || null);

    return [
      { name: "id",             type: sql.Int,     value: item.id ? Number(item.id) : null },
      ...
      { name: "typeofevent",    type: sql.NVarChar,value: finalType },
    ];
  }
},
{
  ...
  query: `
    ...
    WHEN NOT MATCHED THEN
      ...
      VALUES (
        source.typeofevent, source.nameofevent, source.fromDate, source.toDate,
        source.organizer, source.venue, source.sponsor, source.targetAudience, source.employee_id
      );
  `,
  params: item => {
    const finalType = item.typeofevent === "Other"
    ...
    return [
      ...
      { name: "employee_id",    type: sql.NVarChar,value: facultyId },
...

        {
          ...
          `,
          params: item => {
    const finalType = item.membershipType === "Other"
      ? (item.otherType?.trim() || null)
      : (item.membershipType || null);
       return [
            ...
            { name: "membershipType", type: sql.NVarChar, value: finalType || null },
          ];
  }
},
        {
          schema: EmployeeMooc,
          query: `
            MERGE [aittest].[dbo].[EmployeeMooc] AS target
            USING (
              SELECT 
                @id AS id,
                @employee_id AS employee_id,
                @title AS title,
                @typeofMooc AS typeofMooc,
            ...
            WHEN MATCHED THEN
              UPDATE SET
                title = source.title,
                typeofMooc = source.typeofMooc,
                duration = source.duration,
                FDP = source.FDP,
                result = source.result,
                grade = source.grade
            WHEN NOT MATCHED THEN
              INSERT (employee_id, title, typeofMooc, duration, FDP, result, grade)
              VALUES (source.employee_id, source.title, source.typeofMooc, source.duration, source.FDP, source.result, source.grade);
          `,
          params: (item) => {
            const moocType =
              item.typeofMooc === "Others" ? item.otherMoocType || "Other" : item.typeofMooc;

            const duration =
              item.duration === "Others" ? item.otherDuration || "Other" : item.duration;

            return [
              { name: "id", type: sql.Int, value: item.id ? Number(item.id) || null : null },
              { name: "employee_id", type: sql.NVarChar, value: facultyId },
              { name: "title", type: sql.NVarChar, value: item.title || null },
              { name: "typeofMooc", type: sql.NVarChar, value: moocType },
              { name: "duration", type: sql.NVarChar, value: duration },
              { name: "FDP", type: sql.NVarChar, value: item.FDP || "No" },
              { name: "result", type: sql.NVarChar, value: item.result || null },
              { name: "grade", type: sql.NVarChar, value: item.grade || null },
            ];
          },
        },
      ];
        ...
        for (const item of operation.schema) {
          ...
          for (const param of operation.params(item)) {
            if (
              ...
              throw new Error(`Invalid ${param.name} value: ${param.value}`);
            }
      ...
      // Use correct column name for EmployeeMooc
      const idColumn = table === "EmployeeMooc" ? "id" : "id";
      const numericId = Number(id);

      if (isNaN(numericId) || !Number.isInteger(numericId)) {
        return res.status(400).json({
          message: `Invalid ID provided: ${id}. Must be an integer.`,
        });
      }
      ...
      try {
        ...
        if (result.rowsAffected[0] === 0) {
          return res.status(404).json({
            message: `No record found with ${idColumn} ${numericId} in ${mappedTableName}`,
          });
        }

        return res.status(200).json({ message: "Record deleted successfully" });
      } catch (error) {
        console.error(`Error deleting record from ${mappedTableName}:`, {
          id,
          numericId,
          idColumn,
          error: error.message,
        ...
        return res.status(500).json({
          message: `Failed to delete record from ${mappedTableName}`,
          error: error.message,
        });
      }
    } else {
